...
## Project Structure
agent-from-scratch/ ├─ .env ├─ db.json # Local memory database ├─ package.json ├─ tsconfig.json ├─ index.ts # Entry point (CLI) ├─ src/ │ ├─ ai.ts │ ├─ agent.ts │ ├─ llm.ts │ ├─ memory.ts │ ├─ systemPrompt.ts │ ├─ toolRunner.ts │ ├─ ui.ts │ └─ tools/ │ ├─ dadJoke.ts │ ├─ reddit.ts │ ├─ generateImage.ts │ ├─ sentimentAnalysis.ts │ ├─ sentimentTrends.ts │ └─ index.ts # Exports an array of all tool definitions └─ node_modules/
...
2. **Install dependencies**:
   ```bash
   npm install
Set up your .env if needed (for OpenAI API keys).
Usage
Run the agent with a command-line prompt:
...
Dad Jokes (dadjokes.jpg)
Showcasing how the agent fetches a dad joke and returns it in the terminal.

Sentiment Analysis (sentiments.jpg or large-text-analysis.jpg)
Demonstrating simple sentiment or advanced sentiment trend analysis.

Reddit (reddit.jpg)
Showing how the agent fetches and displays the latest front-page posts in JSON format.

Chat-based Memory (chatbased.jpg)
Illustrating how the agent remembers your conversation history.

Image Generation
(Potential screenshot, e.g., imageGen.jpg)

db.json (screenshot of the JSON file or partial snippet)
Highlight how interactions are stored.
